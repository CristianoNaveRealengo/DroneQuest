	...
	AFRAME.registerComponent("game-manager", {
		...
		setupEventListeners: function () {
			...
			this.el.addEventListener(
				"checkpoint-reached",
				this.onCheckpointReached.bind(this)
			);
		...
		setupUI: function () {
			...
			this.uiElements = {
				...
				checkpointCounter: document.querySelector(
					"#checkpoint-counter"
				),
		...
		onCheckpointReached: function (evt) {
			...
			if (
				this.gameState.checkpointsReached >= this.data.totalCheckpoints
			) {
				this.completeLap();
			}
		...
		completeLap: function () {
			...
			console.log(
				...
				} completada em ${this.formatTime(lapTime)}!`
			);

			// Verificar melhor tempo da volta
			if (
				this.gameState.bestLapTime === 0 ||
				lapTime < this.gameState.bestLapTime
			) {
			...
			this.timerInterval = setInterval(() => {
				if (!this.gameState.isPaused) {
					...
					this.gameState.elapsedTime =
						this.gameState.currentTime - this.gameState.startTime;
					this.updateTimer();
				}
			}, 100);
		},
		...
		updateTimer: function () {
			...
			this.gameState.remainingTime =
				this.data.timeLimit * 1000 - this.gameState.elapsedTime;
			...
			if (this.uiElements.timer) {
				...
				this.uiElements.timer.setAttribute(
					"value",
					`TEMPO: ${this.formatTime(timeToShow)}`
				);
		...
		showTimeWarning: function (message, color) {
			...
			warningEl.setAttribute(
				"animation",
				"property: scale; to: 2.5 2.5 2.5; dur: 500; dir: alternate; loop: 3"
			);
		...
		showGameOverScreen: function (title) {
			...
			const stats = [
				...
				`TEMPO DECORRIDO: ${this.formatTime(
					this.gameState.elapsedTime
				)}`,
				"",
		...
		updateUI: function () {
			...
			if (this.uiElements.checkpointCounter) {
				this.uiElements.checkpointCounter.setAttribute(
					"value",
			...
			// Atualizar pontuação
			if (this.uiElements.score) {
				this.uiElements.score.setAttribute(
					"value",
			...
			if (this.uiElements.lapCounter) {
				this.uiElements.lapCounter.setAttribute(
					"value",
			...
			if (this.uiElements.bestTime && this.data.bestTime > 0) {
				this.uiElements.bestTime.setAttribute(
					"value",
		...
		calculateFinalScore: function () {
			...
			const timeBonus = Math.max(
				0,
				(this.data.timeLimit * 1000 - this.gameState.elapsedTime) / 1000
			);
		...
		saveBestTime: function () {
			if (this.gameState.currentLap >= this.gameState.totalLaps) {
				if (
					...
					this.gameState.elapsedTime < this.data.bestTime
				) {
		...
		saveGameData: function () {
			const gameData = {
				...
				totalGames:
					(this.loadedData?.totalGames || 0) +
					(this.gameState.isFinished ? 1 : 0),
			};
		...
		loadGameData: function () {
			try {
				...
				if (saved) {
					...
					this.gameState.bestLapTime =
						this.loadedData.bestLapTime || 0;
				}
			} catch (error) {
		...
		saveHighScore: function () {
			try {
				...
				let highScores = JSON.parse(
					localStorage.getItem("droneRacingHighScores") || "[]"
				);
		...
		showHighScores: function () {
			...
			highScores.forEach((score, index) => {
				...
				console.log(
					`${index + 1}º - ${
						score.score
					} pts | ${timeFormatted} | ${dateFormatted}`
				);
			});
		...
		isNewHighScore: function () {
			...
			return (
				...
				this.gameState.score > highScores[highScores.length - 1].score
			);
		},
