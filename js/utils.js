...
const AudioUtils = {
     ...
     * Cria som sintético
     */
    createSyntheticSound: function(frequency, duration, type = 'sine', volume = 0.3) {
        const ctx = this.init();
    ...
    /**
     * Toca som de motor de drone
     */
    playDroneSound: function(throttle = 0.5) {
        const baseFreq = 100;
        const maxFreq = 400;
        const frequency = MathUtils.lerp(baseFreq, maxFreq, throttle);
        
        return this.createSyntheticSound(frequency, 0.1, 'sawtooth', 0.2);
    },
...
const DebugUtils = {
    ...
    updateDebugPanel: function() {
        ...
        const dronePos = drone ? drone.getAttribute('position') : { x: 0, y: 0, z: 0 };
        
        const debugInfo = [
            `FPS: ${info.fps}`,
            `Entidades: ${info.entities}`,
            `VR: ${info.isVR ? 'Sim' : 'Não'}`,
            `Drone: ${dronePos.x.toFixed(1)}, ${dronePos.y.toFixed(1)}, ${dronePos.z.toFixed(1)}`,
